 ...
 *
 * <p>All payload maps are opaque passthroughs from the CMS GlobalData entry so that the frontend
 * can consume entries by key without ms-home enforcing a rigid schema. Absent CMS fields yield an
 * empty map (never null).</p>
 *
 ...
 * @param header          global navigation header structure (frontend selects logged-in vs.
 *                        logged-out variant based on session state)
 * @param footer          global navigation footer structure
 */
